...
def main(args):
    ...
    # Step 2: Log the loaded model in MLflow under the given model name
    mlflow.sklearn.log_model(
        sk_model=model,
        artifact_path=args.model_name
    )

    # Step 3: Register the logged model using its URI and retrieve its version
    run_id = mlflow.active_run().info.run_id
    model_uri = f"runs:/{run_id}/{args.model_name}"
    mlflow_model = mlflow.register_model(model_uri, args.model_name)
    model_version = mlflow_model.version

    # Step 4: Write registration details to a JSON file in the output path
    print("Writing JSON")
